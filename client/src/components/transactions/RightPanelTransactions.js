...
const RightPanelTransactions = () => {
    ...
    dispatch(setOpenDeleteModal(true))
     ...
     if(Object.keys(userTransactions).length !== 0){
        ...
        // will set desired filter
          .filter(item=>
                    groupingVar === 'day' ? item.day === `${date.format(new Date(), "dddd")}` 
                  : groupingVar === 'week' ? item.week === `Week ${DateTime.now().weekNumber}`
                  : groupingVar === 'month' ? item.month===`${date.format(new Date(),"MMM")}`
                  : groupingVar === 'year' ? item.year === `${date.format(new Date(),"YYYY")}`
                  : null)
          .map(item=>{
              ...
              : `- ${intToString((item.amountInBAM * currencyExchangeRate).toFixed(2))}`
                  ...
                  const thirdRow = <span style={{marginLeft:'30%'}}> 
                    <EditOutlinedIcon fontSize='small' onClick={()=>{editTransaction(item._id)}}/>
                    <DeleteOutlineOutlinedIcon onClick={()=>deleteTransaction(item._id)} fontSize='small' style={{marginLeft:'10%'}} />
            </span>
  ...
  return (
    ...
    { Object.keys(userTransactions).length !== 0 && Object.values(userTransactions.transactions).length < 1 ?
