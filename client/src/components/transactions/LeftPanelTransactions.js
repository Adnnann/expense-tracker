...
import { getUserTransactions, 
        ...
        getGroupingVar, 
        getCurrency,
        setCurrency} from '../../features/usersSlice';
...
const handleChange = (event) => {
    ...
    return (
        ...
        <Box
            ...
            >
                <Typography component="p" style={{textAlign:'center', color: 
                                ...
                                .filter(groupingVar === 'day' ? {'day':`${date.format(new Date(),"dddd")}`}
                                    : groupingVar === 'week' ?  {'week':`Week ${DateTime.now().weekNumber}`}
                                    : groupingVar === 'month' ? {'month':`${date.format(new Date(),"MMM")}`}
                                    : groupingVar === 'year' ? {'year':`${date.format(new Date(),"YYYY")}`}
                                    : null)
                                .groupBy(`${groupingVar}`)
        ...
        : _.chain(Object.values(userTransactions.transactions)
                                ...
                                .isEmpty()
                                .value() ?
          intToString((_.chain(Object.values(userTransactions.transactions)
                                .filter(item=>item.type==='income'))
                                .filter(groupingVar === 'day' ? {'day':`${date.format(new Date(),"dddd")}`}
                                    : groupingVar === 'week' ?  {'week':`Week ${DateTime.now().weekNumber}`}
                                    : groupingVar === 'month' ? {'month':`${date.format(new Date(),"MMM")}`}
                                ...
                                .mapValues(entries => _.sumBy(entries, 'amountInBAM'))
                                .toPairs()
                                .value()[0][1] * currencyExchangeRate).toFixed(2))
            :  _.chain(Object.values(userTransactions.transactions)
                                .filter(item=>item.type==='income'))
                                .filter(groupingVar === 'day' ? {'day':`${date.format(new Date(),"dddd")}`}
                                    : groupingVar === 'week' ?  {'week':`Week ${DateTime.now().weekNumber}`}
          ...
          {Object.keys(userTransactions).length !== 0 && Object.values(userTransactions.transactions).length !== 0 ?   
                    ...
                    income={_.chain(Object.values(userTransactions.transactions).filter(item=>item.type==='income'))
                        .filter(groupingVar === 'day' ? {'day':`${date.format(new Date(),"dddd")}`}
                            : groupingVar === 'week' ?  {'week':`Week ${DateTime.now().weekNumber}`}
                            : groupingVar === 'month' ? {'month':`${date.format(new Date(),"MMM")}`}
                            : groupingVar === 'year' ? {'year':`${date.format(new Date(),"YYYY")}`}
                            ...
                            .value()[0]} 
                    expense={_.chain(Object.values(userTransactions.transactions).filter(item=>item.type==='expense'))
                        .filter(groupingVar === 'day' ? {'day':`${date.format(new Date(),"dddd")}`}
                            : groupingVar === 'week' ?  {'week':`Week ${DateTime.now().weekNumber}`}
                            : groupingVar === 'month' ? {'month':`${date.format(new Date(),"MMM")}`}
                            : groupingVar === 'year' ? {'year':`${date.format(new Date(),"YYYY")}`}
